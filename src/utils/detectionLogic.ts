...
function analyzePaymentGateway(
  ...
  const selectedGateway =
    bangladeshiGateways[
      gatewayKeys[
        lastDigit % gatewayKeys.length
      ] as keyof typeof bangladeshiGateways
    ];

  // Calculate weighted risk score based on gateway performance
  const riskScore =
    selectedGateway.cancellationRate * 0.6 +
    selectedGateway.riskFactor * 100 * 0.4;
...
function validateBangladeshiPhone(phoneNumber: string): {
  isValid: boolean;
  operator: string;
} {
...
export function detectFakeOrder(
  ...
  if (gatewayAnalysis.cancellationRate > 20) {
    reasons.push(
      `High cancellation rate (${gatewayAnalysis.cancellationRate}%) for ${gatewayAnalysis.gateway}`
    );
  ...
  if (gatewayAnalysis.successRate < 80) {
    reasons.push(
      `Low success rate (${gatewayAnalysis.successRate}%) for selected payment method`
    );
    riskScore += 15;
  }
  ...
  if (
    fakeNames.some((name) => customerName.toLowerCase().includes(name)) ||
    commonFakePatterns.test(customerName)
  ) {
  ...
  const hasValidCity = bangladeshiCities.some((city) =>
  ...
  if (
    suspiciousAddressWords.some((word) => address.toLowerCase().includes(word))
  ) {
  ...
  const phoneFrequency = orderHistory.filter(
    (order) => order.phoneNumber === phoneNumber
  ).length;
  if (phoneFrequency > 2) {
    reasons.push(
      `Multiple orders from same phone number (${phoneFrequency} orders) - suspicious pattern`
    );
    riskScore += Math.min(phoneFrequency * 8, 35);
  }

  // Address frequency check
  const addressFrequency = orderHistory.filter(
    (order) =>
      order.address.toLowerCase().trim() === address.toLowerCase().trim()
  ).length;
  if (addressFrequency > 1) {
    reasons.push(
      `Multiple orders from same address (${addressFrequency} orders)`
    );
  ...
  const recentOrders = orderHistory.filter(
    (order) =>
      ...
      new Date().getTime() - new Date(order.timestamp).getTime() <
        2 * 60 * 60 * 1000
  ).length;
  ...
  if (status === "Genuine" && reasons.length === 0) {
    ...
    reasons.push(
      `Payment gateway ${gatewayAnalysis.gateway} has good success rate`
    );
  }
